...
const createField = () => {
  document.body.insertAdjacentHTML(
    "afterbegin",
    "<div class=keyboard> <p class = 'title'>Переключение на LShift + LCtrl</p><div class=keyboard_container></div></div>"
  );
};
createField();
...
class Keyboard {
      ...
      EngLayout[i].map(e => {
        if (typeof e === "number") {
          el.insertAdjacentHTML(
            "beforeend",
            `<div class = "key_btn">${String.fromCharCode(e)}</div>`
          );
        } else {
          el.insertAdjacentHTML(
            "beforeend",
            `<div class = "key_btn spec_key">${e}</div>`
          );
        }
      });
    });
  ...
  addTextarea = () => {
    this.container.insertAdjacentHTML(
      "afterbegin",
      '<textarea class = "textarea"  rows = 10 cols = 10 id = "textarea"></textarea>'
    );
  };
}
...
class Button {
  ...
  LangRus = () => {
    let [shift, ctrlL] = [
      document.querySelector("#ShiftLeft"),
